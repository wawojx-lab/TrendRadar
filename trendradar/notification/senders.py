...
from .wework_app import WeWorkAppSender
...
def send_to_wework_app(
    ...
    *,
    ...
    """
    # 日志前缀
    log_prefix = f"企业微信应用{account_label}" if account_label else "企业微信应用"
    ...
    if is_text_mode:
        print(f"{log_prefix}使用 text 格式推送到个人微信 [{report_type}]")
    else:
        print(f"{log_prefix}使用 markdown 格式推送到个人微信 [{report_type}]")
    ...
    for i, batch_content in enumerate(batches, 1):
        if is_text_mode:
            ...
            plain_content = strip_markdown(batch_content)
            content_size = len(plain_content.encode("utf-8"))
            success = sender.send_text_message(user_id, plain_content)
        else:
            # markdown 格式：保持原样
            content_size = len(batch_content.encode("utf-8"))
            success = sender.send_markdown_message(user_id, batch_content)

        print(f"{log_prefix}第 {i}/{len(batches)} 批次，大小：{content_size} 字节 [{report_type}]")
