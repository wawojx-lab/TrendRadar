...
class NotificationDispatcher:
    ...
    def dispatch_all(
        ...
        # 企业微信应用（直接推送到个人微信）
        if (self.config.get("WEWORK_CORP_ID") and
            self.config.get("WEWORK_AGENT_ID") and
            self.config.get("WEWORK_AGENT_SECRET") and
            self.config.get("WEWORK_USER_ID")):
            results["wework_app"] = self._send_wework_app(
                report_data, report_type, update_info, proxy_url, mode, rss_items, rss_new_items,
                ai_analysis, display_regions, standalone_data
            )
    ...
    def _send_wework_app(
        ...
        """发送到企业微信应用（直接推送到个人微信，支持热榜+RSS合并+AI分析+独立展示区）"""
        rd, ri, rn, ai, sd = self._apply_display_regions(
            report_data, display_regions, rss_items, rss_new_items, ai_analysis, standalone_data
        )
        ...
        agent_secret = self.config.get("WEWORK_AGENT_SECRET", "")
        user_ids = self.config.get("WEWORK_USER_ID", "")

        if not corp_id or not agent_id or not agent_secret or not user_ids:
            print("⚠️  企业微信应用配置不完整，跳过推送")
            print(f"   需要配置: WEWORK_CORP_ID, WEWORK_AGENT_ID, WEWORK_AGENT_SECRET, WEWORK_USER_ID")
            return False

        # 支持多用户推送（用分号分隔）
        user_id_list = parse_multi_account_config(user_ids)
        user_id_list = limit_accounts(user_id_list, self.max_accounts, "企业微信应用")

        results = []
        for i, user_id in enumerate(user_id_list):
            if user_id:
                account_label = f"用户{i+1}" if len(user_id_list) > 1 else ""
                result = send_to_wework_app(
                    corp_id=corp_id,
                    agent_id=agent_id,
                    ...
                    batch_size=self.config.get("MESSAGE_BATCH_SIZE", 4000),
                    batch_interval=self.config.get("BATCH_SEND_INTERVAL", 1.0),
                    msg_type=self.config.get("WEWORK_APP_MSG_TYPE", "text"),
                    split_content_func=self.split_content_func,
                    rss_items=ri,
                    rss_new_items=rn,
                    ai_analysis=ai,
